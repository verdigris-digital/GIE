...
class Quizzes extends lib\BaseCtrl {
  ...
  public function enqueue_scripts() {
    ...
    if($post instanceof \WP_Post && $post->post_type == models\Quiz::$cpt && is_single()) {
      ...
      if($quiz instanceof models\Quiz) {
        ...
        wp_enqueue_script('sortablejs', base\JS_URL . '/vendor/Sortable.min.js', array('jquery'), '1.15.1');
  ...
  public function handle_submit_quiz() {
    ...
    if(!models\UserProgress::has_completed_lesson($user->ID, $quiz->ID)) {
      ...
      \MeprEvent::record( 'mpca-quiz-attempt-completed', $mepr_user, array(
          'attempt_id' => $attempt->id
      ));
    }
      ...
      elseif($question->type == 'fill-blank') {
        if(apply_filters('mpcs_fill_blank_require_all', true)) {
          ...
          if(count($answers) < count($data['answers'])) {
            $errors[] = [
              'id' => $question->id,
              'message' => __('All fields are required', 'memberpress-courses')
            ];
          }
        }
      }
        ...
        if(
          ...
          foreach($values[$question->id] as $value) {
            if(!in_array($value, $question->options, true)) {
              $errors[] = [
                'id' => $question->id,
                'message' => __('Invalid value given', 'memberpress-courses')
              ];
