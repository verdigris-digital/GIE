...
<?php do_action('tutor_assignment/single/before/content'); ?>
    ...
    <div class="tutor-lesson-content-area">
        ...
        <div class="tutor-assignment-information">
            ...
            <ul>
                <li>
					<?php _e('Time Duration : ', 'drivic') ?>
                    <strong><?php echo esc_html($time_duration["value"] ? $time_duration["value"] . ' ' .$time_duration["time"] : __('No limit', 'drivic')); ?></strong>
                </li>
                <!--<li>
                ...
                <li>
					<?php _e('Total Points : ', 'drivic') ?>
                    <strong><?php echo esc_html($total_mark); ?></strong>
                </li>
                <li>
					<?php _e('Minimum Pass Points : ', 'drivic') ?>
        ...
        <div class="tutor-assignment-content">
            <h2><?php _e('Description', 'drivic'); ?></h2>
		...
		if (tutor_utils()->count($assignment_attachments)){
            ...
            <div class="tutor-assignment-attachments">
                <h2><?php _e('Attachments', 'drivic'); ?></h2>
				<?php
		...
		if ($is_submitting){
            ...
            <div class="tutor-assignment-submit-form-wrap">
                <h2><?php _e('Assignment answer form', 'drivic'); ?></h2>
                ...
                <form action="" method="post" id="tutor_assignment_submit_form" enctype="multipart/form-data">
                    ...
                    <div class="tutor-form-group">
                        <p><?php _e('Write your answer briefly', 'drivic'); ?></p>
					...
					<?php if ($allowd_upload_files){ ?>
                        <p><?php _e('Attach assignment files', 'drivic'); ?></p>
                        <div class="tutor-assignment-attachment-upload-wrap">
							<?php
							for ($item = 1; $item <= $allowd_upload_files; $item++){
								?>
                                <div class="tutor-form-group">
                                    <label for="tutor-assignment-input-<?php echo esc_html($item); ?>"><i class="tutor-icon-upload-file"></i><span><?php _e('Upload file', 'drivic'); ?></span></label>
                                    <input class="tutor-assignment-file-upload"  id="tutor-assignment-input-<?php echo esc_html($item); ?>" type="file" name="attached_assignment_files[]">
                                </div>
								<?php
							}
							?>
                    ...
                    <div class="tutor-assignment-submit-btn-wrap">
                        <button type="submit" class="tutor-button tutor-success" id="tutor_assignment_submit_btn"> <?php _e('Submit Assignment', 'drivic');
							?> </button>
                    </div>
                </form>
			...
			if ($submitted_assignment){
				...
				if ($is_reviewed_by_instructor){
                    ...
                    <div class="assignment-result-wrap">
                        <h4><?php echo sprintf(__('You received %s points out of %s', 'drivic'), "<span class='received-marks'>{$given_mark}</span>", "<span class='out-of-marks'>{$max_mark}</span>") ?></h4>
                        <h4 class="submitted-assignment-grade">
							<?php _e('Your Grade is ', 'drivic'); ?>
							<?php if ($given_mark >= $pass_mark){
								?>
                                <span class="submitted-assignment-grade-pass">
                                    <?php _e('Passed', 'drivic'); ?>
                                </span>
								<?php
							}else{
								?>
                                <span class="submitted-assignment-grade-failed">
                                    <?php _e('Failed', 'drivic'); ?>
                                </span>
								<?php
							} ?>
                        </h4>
                ...
                <div class="tutor-assignments-submitted-answers-wrap">
                    ...
                    <h2><?php _e('Your Answers', 'drivic'); ?></h2>
					...
					if ($attached_files){
						...
						if (tutor_utils()->count($attached_files)){
							...
							?>
                            <h2><?php _e('Your uploaded file(s)', 'drivic'); ?></h2>
							...
							foreach ($attached_files as $attached_file){
                                ...
                                <div class="uploaded-files">
                                    <a href="<?php echo esc_html($upload_baseurl.tutor_utils()->array_get('uploaded_path', $attached_file)) ?>" target="_blank"><?php echo tutor_utils()->array_get('name', $attached_file); ?>
                                    </a>
                                </div>
								<?php
							}
					...
					if ($is_reviewed_by_instructor){
                        ...
                        <div class="instructor-note-wrap">
                            <h2><?php _e('Instructor Note', 'drivic'); ?></h2>
                ...
                <div class="tutor-assignment-start-btn-wrap">
                    <form action="" method="post" id="tutor_assignment_start_form">
                        ...
                        <button type="submit" class="tutor-button" id="tutor_assignment_start_btn"> <?php _e( 'Start assignment submit', 'drivic' ); ?> </button>
                    </form>
                </div>
				<?php
			}
